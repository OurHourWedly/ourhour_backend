...
class InvitationViewSet(viewsets.ModelViewSet):
    ...
    def rsvps(self, request, pk=None):
        ...
        """
        invitation = get_object_or_404(Invitation, pk=pk, status="PUBLISHED")

        if request.method == "POST":
            # RSVP 생성 (인증 불필요)
            if not invitation.enable_rsvp:
                return Response(
                    {"error": "이 청첩장은 RSVP 기능이 비활성화되어 있습니다."}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            if serializer.is_valid():
                try:
                    ...
                    existing_rsvp = RSVP.objects.filter(
                        invitation=invitation, guest_name=guest_name, phone=phone
                    ).first()
    ...
    def guestbooks(self, request, pk=None):
        ...
        if request.method == "POST":
            ...
            if not invitation.enable_guestbook:
                return Response(
                    {"error": "이 청첩장은 방명록 기능이 비활성화되어 있습니다."}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            # 방명록 목록 조회 (공개만)
            guestbooks = Guestbook.objects.filter(invitation=invitation, is_public=True)
            paginator = PageNumberPagination()
            paginator.page_size = 20
...
class PublicInvitationView(APIView):
    ...
    def get(self, request, slug):
        invitation = get_object_or_404(Invitation, url_slug=slug, is_public=True, status="PUBLISHED")
